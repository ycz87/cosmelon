...
### 仓库与合成
- **瓜棚双 Tab** — 🍉 瓜棚（收获物 + 合成 + 切瓜）/ 🎒 背包（种子 + 道具），Segmented Control 切换
...
### 农场系统 🌱
- **五星系** — 厚土/烈火/寒水/青木/玄金，收集 5 品种解锁下一星系
- **40 个品种** — 4 种稀有度，每个品种有独特故事和 emoji
- **俯视微倾斜瓜田** — 3×3 网格，始终显示 9 块地（未开发锁定），点击种植/收获
- **离线生长** — 关闭浏览器后西瓜按分钟继续生长
- **图鉴系统** — 按星系分页，品种详情弹窗，星际旅程进度追踪
- **地块扩展** — 里程碑解锁更多地块（最多 9 块）
- 品种揭晓动画：rare 及以上稀有度拥有旋转光带、粒子环与强化文本发光
- 收获动画分流：新品种全屏烟花粒子；重复品种低调展示
- **基因片段系统** — 收获品种获得基因片段，实验室 tab 按星系分组查看背包
- **基因注入系统** — 将基因片段注入种子，定向培育目标星系品种；种植弹窗支持注入种子选项
- **双元素融合** — 两个不同星系基因融合产出杂交种子（5档成功率 + 基因改造液+20%）
- **杂交图鉴** — 30个杂交品种，按10个星系组合分组，已获得/未获得

### 成就徽章 🏆
- **44 个成就** — 5 个系列：⭐️坚持 / ⏱️专注 / 🏠瓜棚 / 🌱农场 / 🌟隐藏
- **真实徽章图片** — 44 张 AI 生成 PNG 徽章（512×512 透明背景），已解锁显示彩色，未解锁显示灰色轮廓，图片加载失败自动 fallback 到 emoji
- 顶栏🏆成就按钮，未查看新解锁红点提示
- 按系列分组展示，总进度条 + 系列进度条
- 徽章详情弹窗：名称、描述、解锁条件、进度条、96px 大尺寸徽章
- 解锁庆祝动画：灰色→彩色真实图片 + 光波 + 金色粒子
- 隐藏成就：未解锁显示"???"+❓图标，解锁条件不可见
- 瓜棚系列：收获物收集、金西瓜、合成、切瓜、道具收集自动检测
- 农场系列：种植、收获、品种收集已接入；外星人、瓜贼、连续活跃检测为后续扩展
