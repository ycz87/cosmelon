...
### 新增
- 农场数据层重构：5 大星系（厚土/烈火/寒水/青木/玄金）、40 个品种定义、分钟级生长引擎
- 等距菱形瓜田 UI：伪 3D 钻石网格布局，点击种植/收获交互
- 星系解锁链：收集 5 个品种解锁下一星系，地块里程碑扩展（3/5/8/15/22 品种）
- 图鉴页面升级：按星系分页展示，未解锁星系锁定状态
- 品种详情弹窗：点击已收集品种查看故事、首次获得日期、累计收获次数
- 星际旅程进度面板：整体收集进度条 + 各星系独立进度追踪
- 40 个品种的 i18n 翻译（名称 + 故事），覆盖全部 8 种语言
- 收获时种子奖励机制

### 改进
- CollectionPage 性能优化：collection 数组改为 Map 查找，避免 O(n) find
- VarietyCard 改为 button 元素，提升可访问性
...
### 改进
- 中文「番茄钟」改为「番茄模式」，与「项目模式」对称
- 移除时钟上方的 phase label capsule（"🍉 专注时间"/"☕ 休息一下"胶囊标签），界面更简洁
...
### 改进
- 🏆 成就按钮从设置面板移至顶栏，位于瓜棚和历史之间
- 顶栏按钮顺序：🏠瓜棚 → 🏆成就 → 📅历史 → ⚙️设置
- 未读红点样式保持不变
- Settings 组件清理：移除 onShowAchievements / achievementUnseenCount props
...
### 新增
- 🌱 农场系列 8 个成就检测逻辑预埋（G1-G8）
- `detectFarmAchievements()` — 独立的农场成就检测函数
- useAchievements 新增 `checkFarm()` 方法（等农场功能上线后接入）
- AchievementProgress 新增 7 个农场追踪字段（totalPlants, totalFarmHarvests, alienVisits, thiefDefenses, farmActiveStreak, completedGalaxies, totalVarieties）
- 农场系列 G1-G8 补全中英文描述、解锁条件、进度条配置
- 农场系列移除"即将开放"标记，徽章可正常交互
...
### 新增
- 🏆 成就系统：44 个成就，5 个系列（⭐️坚持 / ⏱️专注 / 🏠瓜棚 / 🌱农场 / 🌟隐藏）
- 成就页面：设置面板"🏆 成就"入口，按系列分组展示，总进度条 + 系列进度条
- 徽章展示：未解锁灰色 + 🔒，已解锁系列主色调 + 微光效果，隐藏系列显示"???"
- 徽章详情弹窗：点击查看名称、描述、解锁条件、进度条、解锁时间
- 解锁检测：坚持系列 10 个 + 专注系列 10 个全部实现（连续打卡、累计时长、单次深度、时间段等）
- 隐藏系列 6 个检测（午夜、情人节、完美主义者等）
- 解锁庆祝弹窗：灰色→彩色动画 + 光波扩散 + 金色粒子，多个依次展示
- 未查看新解锁红点提示
- 瓜棚系列和农场系列展示但标注"即将开放"
- 中英文双语支持（8 种语言 i18n 扩展）
- localStorage 持久化成就数据

### 技术细节
- `src/achievements/` — 类型定义、44 个成就定义、检测逻辑
- `src/hooks/useAchievements.ts` — 成就状态管理 hook
- `src/components/AchievementsPage.tsx` — 成就页面 UI
- `src/components/AchievementCelebration.tsx` — 解锁庆祝动画
- 检测时机：专注完成时、每日首次打开时
- 成就进度追踪：连续打卡、累计天数、单日次数、周末战士等
...
### 新增
- 6 种新语言：日语 🇯🇵、韩语 🇰🇷、西班牙语 🇪🇸、法语 🇫🇷、德语 🇩🇪、巴西葡萄牙语 🇧🇷
- 语言选择弹窗：国旗 + 语言原名列表，当前选中高亮，5 套主题适配
- 浏览器语言自动检测扩展到 8 种语言

### 改动
- Settings 语言切换从双按钮改为弹窗选择器
- detectLocale 默认回退从中文改为英文（国际化更合理）
...
### 修复
- Toast 改为浮动定位（absolute），不再占用布局空间，不影响下方按钮位置
- 快捷选择器选 ≤25min 时立即隐藏正在显示的 Toast（不用等 3.5s 自然消失）
...
### 新增
- 快捷时间选择器选 >25min 时弹出 Toast 健康提醒
- 新增 Toast 组件：fade-in / fade-out 动画，3500ms 自动消失
- 适配 5 套主题（Dark/Light/Forest/Ocean/Warm）
- 文案使用 i18n（中："超过 25 分钟需手动结束，记得适时休息 🧘"）
- healthToast 独立 state，不与长按提示共用
- ≤25min 不弹 Toast
