...
- 计时器运行中禁止切换 Tab
- 番茄模式/项目模式切换（ModeSwitch）在专注 Tab 内时钟区域上方
...
### 农场系统（Phase 2）
- 5 大星系：厚土星/烈火星/寒水星/青木星/玄金星，收集 5 品种解锁下一星系
- 40 个品种：4 种稀有度（普通/稀有/史诗/传说），每个品种有独特故事
- 俯视微倾斜 3×3 瓜田：CSS Grid + perspective 伪3D，始终显示 9 块地（未开发锁定）
- 分钟级离线生长：关闭浏览器后西瓜继续生长
- 图鉴系统：按星系分页展示收集进度，品种详情弹窗（故事/首次获得/累计收获）
- 星际旅程：整体收集进度 + 各星系独立进度追踪面板
- 地块扩展：里程碑解锁（3/5/8/15/22 品种对应 5/6/7/8/9 地块）
...
| v0.17.0 | ✅ 完成 | 成就徽章系统 — 44 个成就、5 系列、检测逻辑、庆祝动画 |
| v0.17.1 | ✅ 完成 | 瓜棚系列成就检测 — 10 个瓜棚成就自动检测 |
| v0.17.2 | ✅ 完成 | 农场系列成就检测预埋 — 8 个农场成就检测逻辑 |
| v0.18.0 | ✅ 完成 | 成就系统完整版 — 隐藏系列完善 + 云端同步前端逻辑 |
| v0.18.1 | ✅ 完成 | 成就按钮移至顶栏 — 提升可发现性 |
| v0.19.0 | ✅ 完成 | 顶部导航栏重构 — 三 Tab + 工具图标 |
| v0.19.1 | ✅ 完成 | UI 微调 — 番茄模式命名 + 移除 phase capsule |
| v0.19.2 | ✅ 完成 | 修复 auth 服务 FRONTEND_URL |
| v0.19.3 | ✅ 完成 | 成就数据云端同步 API — 后端 achievements 端点 + D1 表 |
| v0.20.0 | ✅ 完成 | 徽章图片 — 44 张真实 PNG 替换 emoji 占位 |
| v0.21.x | ✅ 完成 | 切瓜系统 + 瓜棚改版 — 切瓜交互/瓜棚双 Tab/背包 |
| v0.22.0 | ✅ 完成 | 农场 Phase 2 — 五星系/等距瓜田/图鉴升级/品种详情/星际旅程 |
| v0.23.0 | ✅ 完成 | 瓜田布局改版 — 俯视微倾斜 3×3 网格/锁定地块/圆角矩形 |
| v0.8 | ⏳ 计划 | 切瓜（待定） |
