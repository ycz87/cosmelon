...
**视觉风格：** 俯视微倾斜（Top-down with slight tilt，约 15°-20° CSS perspective + rotateX），地块用圆角矩形。

**固定 3×3 网格（始终显示 9 块地）：**
- 始终渲染 9 块地，让玩家一眼看到全部空间
- 已开发地块：正常交互（种植/生长/收获）
- 未开发地块：灰显/锁定状态，显示 🔒 + 解锁条件提示
- 初始已开发 4 块，其余 5 块锁定

**响应式适配：**
- 竖屏（手机）：3列×3行，地块紧凑排布，整体加轻微 perspective 倾斜出伪3D感
- 横屏（PC）：同样 3×3，地块可稍大，两侧自然留白
- 地块大小用 CSS clamp() 自适应

**解锁进度（双轨：里程碑免费送 或 瓜币购买，取先到的）：**
