...
### Fixed
- E-001-T11 二次返工：默认验收画面改为 7 地块三状态并存（空地 + 幼苗 + 成熟西瓜）
- E-001-T11 二次返工：成熟地块占比上调且默认呈现 4 西瓜/组，避免画面以 +Plant 占位为主

## [0.61.1] - 2026-02-25

### Fixed
- E-001-T11 Rework：7 地块进一步打散成不规则错位布局，弱化规则菱形阵列感
- E-001-T11 Rework：左上橙色双层房屋与右上红色谷仓继续放大，提升与参考图一致的视觉权重
- E-001-T11 Rework：右上谷仓旁木箱 + 小鸡增强为可识别主元素（尺寸与层级上调）
- E-001-T11 Rework：成熟地块 4 西瓜/组展示进一步强化，并保持空地/幼苗/成熟三态同屏

## [0.61.0] - 2026-02-25

### Changed
- 农场视觉复刻补齐 4 项核心差异：地块改为不规则错位排列，布局更接近参考图自然分布
- 左上房屋升级为橙色双层带烟囱样式；右上建筑升级为红色拱顶谷仓样式
- 右上配景补齐木箱与小鸡元素，场景信息密度与参考图对齐
- 成熟地块改为 4 西瓜一组展示，画面状态分布调整为“空地+幼苗+成熟果实”组合

## [0.60.2] - 2026-02-24

### Fixed
- E-001-T11 第三次返工：验收口径切换为 7 块地（不再 4+3 锁定）
- E-001-T11 第三次返工：地块群继续下移，桌面/移动均稳定落在山体下方
- E-001-T11 第三次返工：地块横纵间距继续压缩，排布更密且不重叠

## [0.60.1] - 2026-02-24

### Fixed
- E-001-T11 二次返工：地块群继续下移到山体下方，消除“进入天空区”视觉
- E-001-T11 二次返工：地块横纵间距进一步压缩，地块排布更密但保持不重叠

## [0.60.0] - 2026-02-24

### Changed
- 完成 E-001 视觉打磨终验收口（T02~T10）：构图、地块体积、接地融合、四角配景、手绘一致性、多端响应式全部闭环
- 农场地块风格统一：主/细描边节奏、边缘软硬、顶面颗粒密度收敛，显著降低“拼装感”
- 移动端安全区与裁切边界收口：小屏布局稳定，核心锚点在桌面/移动端均可见

### Technical
- 新增终验对比入口：`npm run compare:e001-t10`
- 输出终验报告与可复盘材料（评分模型 + 三联对比 + 发布信息）

## [0.59.0] - 2026-02-23

### Added
- VisualOnly 模式（`?visualOnly=1`）：隐藏统计卡片与日志面板，仅展示农场主画面
- 农场 Pixi 原型 Step 6 最终交付：构图/配色/元素细节三维度相似度 86.2%（>=85 达标）

### Changed
- 地块间距微调，减少聚合感，构图更接近参考图
- 相机位置上移，主画面重心对齐参考图
...
- 左上角可爱太阳：SVG 黄色圆形 + 10条光芒 + 笑脸
- 4朵蓬松白云：SVG 圆形组合 + 浅灰边框
...
- 配色升级：天空 #D4F1F9→#E8F8FC，草地 #DDED92→#C8E66D，土壤 #D4956C→#B8754A
- 边框增强：3px solid #A0643D，更清晰的地块边界

## [0.52.0] - 2026-02-22

### Fixed
- 优化 night 天气叠加层透明度（0.65→0.3），保留底层明亮配色可见性
- 移除 FarmPage filter 样式，避免压暗明亮配色

## [0.51.0] - 2026-02-22

### Changed
- 农场布局重构：从 3x3 九宫格改为 7 块等距菱形布局
- 农场视觉升级：明亮草地 + 蓝天背景，独立土壤岛设计
- 地块上限调整：最多 7 块（4->5->6->7）

### Technical
- 新增 IsometricFarmGrid.tsx 组件（2.5D 等距视角）
- 新增 FarmEnvironment.tsx 背景组件
- 更新 PLOT_MILESTONES 数据结构
- PlotCard 导出为可复用组件
